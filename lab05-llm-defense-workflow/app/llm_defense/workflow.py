...
question_check_agent = client.create_agent(
    ...
    instructions="""
        ...
        Task: classify the user's message into exactly ONE intent:
        - greeting
        - goodbye
        - weather
        - geography
        ...
        Output MUST be ONLY valid JSON, exactly:
...
def is_allowed(expected_result: bool):
    ...
    def condition(message: AgentExecutorResponse) -> bool:
        try:
            intent = QuestionCheckResult.model_validate_json(
                message.agent_run_response.text
            ).intent
        except Exception:
            # FAIL CLOSED: anything invalid goes to refusal
            intent = "other"

        is_eligible = intent in (
            "greeting",
            "goodbye",
            "weather",
            "geography"
